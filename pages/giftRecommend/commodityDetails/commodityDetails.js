// pages/giftRecommend/commodityDetails/commodityDetails.js
const app = getApp()
...
Page({
  ...
  getCollectStatus: function() {
    ...
    app.agriknow.postIsCollect(token, productId).then(res => {
      if (res.status == 200) {
        let data = res.bean
        if (data == 1) {
          that.setData({
  ...
  toogleCollect: function() {
    ...
    if (coll == false) {
      ...
      app.agriknow.postCancelCollect(token, productId).then(res => {
        if (res.status == 200) {
          that.setData({
            collect: false
          })
  ...
  onShareAppMessage: function() {
    ...
    let id = that.data.id
    return{
      path:'pages/giftRecommend/commodityDetails/commodityDetails?id='+id,
