...
Page({
  ...
  getCanAssistant: function () {
    wx.request({
      url: config.config.getCanAssistantActivityAnswer,
      data: {
        token: this.data.token,
        ordersId: this.data.ordersId,
  ...
  assistant: function () {
    wx.request({
      url: config.config.postActivityAnswerAssistance,
      data: {
        token: this.data.token,
        ordersId: this.data.ordersId,
        shareUserId: this.data.shareUserId
    ...
    wx.request({
      ...
      success: (res) => {
        if (res.data.status == 200) {
          ...
          // })
          wx.navigateTo({
            url: '/packageA/pages/activityAnswerGameCenter/activityAnswerGameCenter?ordersId=' + this.data.ordersId + '&qustionNumber=' + this.data.qustionNumber + '&userId=' + userId,
          })
        } else if (res.data.status == 801) {
    ...
    if (res.from === 'button') {
      if (res.target.id == 'getRelive') {
        ...
        return {
          ...
          path: '/packageA/pages/assistanceActivityAnswer/assistanceActivityAnswer?ordersId=' + this.data.ordersId + '&shareUserId=' + this.data.userId,
          imageUrl: 'https://image.prise.shop/images/2018/07/05/1530775084567947.png'
        }
      } else if (res.target.id == 'joinGame') {
        return {
          title: '答题有奖，够快够准就能稳赢！',
          path: '/packageA/pages/activityAnswer/activityAnswer?ordersId=' + this.data.ordersId,
